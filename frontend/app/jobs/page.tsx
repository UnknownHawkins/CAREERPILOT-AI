...
import { JobFinder } from '@/components/dashboard';
...
export default function JobMatchPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = (await jobMatchApi.createMatch({
        ...formData,
      ...
      const message = error.response?.data?.message || 'Failed to analyze job match';
      
      toast.error(
        isLimitError ? 'Limit Reached' : 'Analysis Error', 
        message + (isLimitError ? '. Upgrade to Pro in the sidebar for unlimited matches!' : '')
      );
